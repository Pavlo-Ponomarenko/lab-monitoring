resource "aws_ecs_task_definition" "web" {
  ...
  container_definitions = jsonencode([
    {
      name      = "web-app"
      image     = "nginx:alpine" # Для тесту; у продакшені тут ваш додаток з ендпоїнтом /metrics
      essential = true
      portMappings = [
        {
          name          = "http-web"
...
resource "aws_ecs_service" "web" {
  ...
  load_balancer {
    ...
    container_name   = "web-app" # має збігатися з іменем контейнера в Task Definition
    container_port   = 80
  }
}
